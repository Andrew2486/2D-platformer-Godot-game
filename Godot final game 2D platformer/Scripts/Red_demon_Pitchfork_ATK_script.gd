...
func _on_Area2D_body_entered(body):
	if body.name == "Player":
		body.position = spawn.position#replaces set cords
		#body.position = spawn.position#replaces set cords
		get_node("/root/MainHudScene").lives -= 1
		$AudioStreamPlayer.play()


func _on_Area_of_sight_body_entered(body):#TOFIX When player enters update every tick to avoid cheesing jump
	if body.name == "Player":
		target = Player
